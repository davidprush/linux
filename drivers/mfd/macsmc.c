...
/* We don't actually know the true limits here but these seem reasonable */
				...
				const void *wbuf, size_t wsize)
{
	...
	if (wsize) {
		cmd = rsize ? SMC_MSG_RW_KEY : SMC_MSG_WRITE_KEY;
	...
	ret = apple_smc_cmd_locked(smc, cmd, key, rsize, wsize, &rdata);
	if (ret < 0)
		return ret;
	...
	void *rbuf, size_t rsize)
{
	...
	return apple_smc_rw_locked(smc, key, rbuf, rsize, wbuf, wsize);
}
			       ...
			       int scale)
{
	...
	/*
	 * val is a 48.16 fixed point number, hence dividing by 1<<16 results
	 * in an intger containing only the digits while dropping the fractional
	 * part. Multiplying by scale first shifts part of the fraction to the
	 * digits. mult_frac takes care of both at the same time and avoids
	 * unnecessary overflows or loss of precision.
	 */
...
int apple_smc_enter_atomic(struct apple_smc *smc)
{
	 ...
	 */
	const u8 flag = 0;
	apple_smc_rw_locked(smc, SMC_KEY(NTAP), NULL, 0, &flag, sizeof(flag));
...
static int apple_smc_platform_probe(struct platform_device *pdev)
{
	...
	ret = mfd_add_devices(smc->dev, -1, apple_smc_devs, ARRAY_SIZE(apple_smc_devs), NULL, 0, NULL);
	if (ret) {

...
static void macsmc_gpio_set(struct gpio_chip *gc, unsigned int offset, int value)
{
	...
	if (ret < 0)
		dev_err(smcgp->dev, "GPIO set failed " SMC_KEY_FMT " = 0x%x\n",
			SMC_KEY_ARG(key), value);
}
				       ...
				       unsigned long *valid_mask, unsigned int ngpios)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (gpio_nr < 0 || gpio_nr > MAX_GPIO) {
			dev_err(smcgp->dev, "Bad GPIO key " SMC_KEY_FMT "\n",
				SMC_KEY_ARG(key));
			continue;
		}
...
static int macsmc_gpio_probe(struct platform_device *pdev)
{
	...
	dev_info(smcgp->dev, "First GPIO key: " SMC_KEY_FMT "\n", SMC_KEY_ARG(key));

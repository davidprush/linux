 ...
 * endianess is used when these are stored to memory and sent to or received
...
/**
 * SMC_KEY_FMT - printf string for smc_key
 */
#define SMC_KEY_FMT "%c%c%c%c (0x%08x)"

/**
 * SMC_KEY_ARG - printf argument for smc_key
 *
 * @k: smc_key
 */
 ...
 * @be: pointer to to backend functions (e.g. RTKit for Apple Silicon)
...
static inline bool apple_smc_write_flag(struct apple_smc *smc, smc_key key, bool state)
{
	return apple_smc_write_u8(smc, key, state ? 1 : 0);
}

static inline bool apple_smc_write_flag_atomic(struct apple_smc *smc, smc_key key, bool state)
{
 ...
 * degrees as a 48.16 fixed point number, and scale it to TODO like so:
 *
 * 	TR0Z
 *
